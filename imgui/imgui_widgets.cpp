...
bool ImGui::Tab(const char* icon, int* tab, int number)
{
    ...
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, *tab == number ? ImColor(1.f, 1.f, 1.f, 0.05f) : ImColor(1.f, 1.f, 1.f, 0.f), g.IO.DeltaTime * anim_speed);
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (ColorButton("##ColorButton", col_v4, flags))
        {
            if (!(flags & ImGuiColorEditFlags_NoPicker))
            {
                ...
                OpenPopup("picker");
                SetNextWindowPos(main_window->Pos + ImVec2(450 + picker_size, 0));
            }
        }
        if (!(flags & ImGuiColorEditFlags_NoOptions))
